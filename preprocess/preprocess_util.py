...
def get_encodings(location=os.path.join('training_data', 'known'), file_name_transform=lambda x: x, num_jitters=1):
...
def get_face_pics(location=os.path.join('training_data', 'known'), file_name_transform=lambda x: x, face_only=True,
                  resize_to_square=None):
